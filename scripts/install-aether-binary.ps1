...
# into your Downloads folder (or pass -Source), then run this script from the repo root:
#
#   aether-android-arm64.tar.gz
#   aether-android-armv7.tar.gz
#
#   powershell -ExecutionPolicy Bypass -File android\scripts\install-aether-binary.ps1

param(
    [string]$Source = "$env:USERPROFILE\Downloads",
    [string]$JniLibs = (Join-Path (Split-Path $PSScriptRoot -Parent) "app\src\main\jniLibs")
)
