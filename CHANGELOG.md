...
- سایت‌های تحریمی (که خودِ ابزار استعلام همین بخش هم «تحریم» تشخیصشان می‌داد) بازهم باز نمی‌شدند و خطای ۴۰۳ می‌دادند، دقیقاً انگار این قابلیت اصلاً روشن نبود. علت (پیدا شده با گرفتن لاگ زنده از یک گوشی واقعی): مسیریابی دامنه‌ها کاملاً به تشخیص SNI از TLS ClientHello وابسته بود — که برای هر سایتی که از Encrypted Client Hello (ECH) استفاده کند (که این روزها پیش‌فرض کروم است) کاملاً از کار می‌افتد، چون SNI رمزنگاری شده و قابل خواندن نیست. نتیجه این‌که هر اتصال «تحریمی» به قانون پیش‌فرض «مستقیم» می‌افتاد و از همان آی‌پی واقعی ایران به مقصد می‌رسید. رفع شد با استفاده از یک استخر DNS جعلی (fakedns) مخصوص دامنه‌های تحریمی به‌جای اتکا به sniffing — این روش کاملاً مستقل از TLS handshake کار می‌کند و در برابر ECH هم مقاوم است.
